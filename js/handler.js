...
Math.seedrandom(Math.floor(Math.random() * 100));
...
$(document).ready(function() {
	...
	if(localStorage.getItem('candy_board') == null){
		//ClearCanvas();
		board.score = 0;
		//alert("none");
	}else{	
		board.score = parseInt(localStorage.getItem('candy_board'));
	}

	var scoreBox = document.getElementById("scoreBox");
	var color_ ="blue";
	if(localStorage.getItem('candy_color') == null){
		$(scoreBox).css("color", "lightgrey");
	}else{	
		color_ = localStorage.getItem('candy_color');
	}

	var candyIcon = document.getElementById("candyIcon");
	var scoreLabel = document.getElementById("scoreLabel");
	
	$(scoreLabel).empty();
	$(scoreLabel).append(board.score);
	$(scoreBox).css("background-color", color_);

	if(color_ == "yellow"){
		$(scoreBox).css("color", "gray");
			//candyIcon.src = "image/" + candy.toString() + "-candy.png";
	}else{
		$(scoreBox).css("color", "white");
	}

	candyIcon.src = "image/" + color_ + "-candy.png";
...
$(board).on('add', function(e, info) {
	...
	img.src = "image/" + candy.toString() + "-candy.png";
	
	$(img).data("candy", candy);
	$(img).attr("id", "candy-id-" + candy.id);
	$(img).attr("class", "candy-id");
	$(img).attr("data-position", candy.col + "-" + candy.row);
	
	var candySize = boardSize/board.boardSize;
...
$(board).on('scoreUpdate', function(e, info) {
	...
	localStorage.setItem('candy_board', info.score);
	localStorage.setItem('candy_color', info.candy.color);
	
	if(info.candy != undefined){
		$(scoreBox).css("background-color", info.candy.color);
		...
		$(scoreBox).css({"background-color" : "lightgrey",
							"color" : "black"});
	}

	candyIcon.src = "image/" + info.candy.toString() + "-candy.png";
});
...
$(document).on("mouseup touchend", function(evt){
	if (dragDropInfo != null){
		...
		//var savedata = JSON.stringify(board.square);
		//localStorage.setItem('candy_board', savedata); 
...
function DrawArrow(){
	...
	if(validMove.direction == "up"){
		...
		ctx.strokeRect(x - (squareSize), y- (squareSize*3), squareSize*2, squareSize*4);
		...
		*/
		...
		ctx.strokeRect(x - (squareSize), y- (squareSize), squareSize*2, squareSize*4);
		...
		*/
		...
		ctx.strokeRect(x- (squareSize*3) , y - (squareSize), squareSize*4, squareSize*2);
		...
		*/
		...
		ctx.strokeRect(x- (squareSize), y - (squareSize), squareSize*4, squareSize*2);
	...
	/*
	var img = new Image;
	  img.src = "image/arrow_"+validMove.direction+".png";
	  img.onload = function() {
	    ctx.drawImage(img, (col) * candySize - squareSize-1, (row) * candySize- squareSize-1);
	  };
	*/
